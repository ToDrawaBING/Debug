...
void Tower::checkEnemyInRange()
{
    if (m_chooseEnemy)
    {
        ...
        // 如果敌人脱离攻击范围
        if (!collision(m_pos, m_attackRange, m_chooseEnemy->getPos(), 1))
            lostSightOfEnemy();
    }
    else
    {
        // 遍历敌人,看是否有敌人在攻击范围内
        QList<Enemy *> enemyList = m_game->_EnemyList();
        foreach (Enemy *enemy, enemyList)
        {
            if (collision(m_pos, m_attackRange, enemy->getPos(), 1))
            {
...
{
    Bullet *bullet = new Bullet(m_pos, m_chooseEnemy->getPos(), m_damage, m_chooseEnemy, m_game);
    bullet->move();
